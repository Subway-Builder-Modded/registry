...
test("download-only mode preserves previous downloads when a GitHub repo is private or deleted", async () => {
  await withTempRegistry(async ({ repoRoot, writeIndex, writeManifest }) => {
    writeIndex("mods", ["github-mod"]);
    writeIndex("maps", []);
    writeManifest("mods", "github-mod", {
      ...makeBaseModManifest("github-mod"),
      update: { type: "github", repo: "owner/gone" },
    });
    writeJson(join(repoRoot, "mods", "downloads.json"), {
      "github-mod": {
        "v2.0.0": 14,
        ...
        "github-mod": {
          ...
          versions: {
            "v2.0.0": {
              ...
              required_checks: {},
              matched_files: {},
              source: { update_type: "github", repo: "owner/gone", tag: "v2.0.0" },
              fingerprint: "fp",
              checked_at: "2026-03-31T00:00:00.000Z",
            },
          },
    ...
    const fetchMock = makeFetchRouter([
      {
        ...
        handle: () => Response.json({
          ...
          errors: [{ message: "Could not resolve to a Repository with the name 'owner/gone'." }],
        }),
      },
    ]);
...
test("full mode preserves previous integrity and downloads when a GitHub repo is private or deleted", async () => {
  await withTempRegistry(async ({ repoRoot, writeIndex, writeManifest }) => {
    writeIndex("mods", ["github-mod"]);
    writeIndex("maps", []);
    writeManifest("mods", "github-mod", {
    ...
    assert.equal(integrity.listings["github-mod"]?.versions["v2.0.0"]?.is_complete, true);
    assert.ok(
      warnings.some((warning) => warning.includes("preserved previous integrity and download state (repository not found or inaccessible)")),
    );
  });
});
