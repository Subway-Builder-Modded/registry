...
const dataQualityTemplateBaseDoc: TemplateDoc = {
  ...
  body: [
    ...
    spacer(),
    markdown(
      "## Workplace data\n\nWhere people **work** — the destination side of every commute, and the largest part of the score.",
    ),
    dropdown(
      "dq-workplace-source",
      "Where do your job numbers come from?",
      "The key distinction: does your source know where people actually work, or only where their employer is registered? See the guide for examples.",
    ...
    dropdown(
      ...
      "Answer with the smallest area in your source's own table — not how finely the finished map places demand points (the next question covers that).",
    ...
    dropdown(
      ...
      "How totals were spread below the published grain. If unsure, describe the approach in Methodology instead.",
      WORKPLACE_PLACEMENT_FORM_OPTIONS.map((option) => option.formLabel),
      false,
    ),
    spacer(),
    markdown(
      "## Residence data\n\nWhere people **live** — the origin side of the commute. Check the guide's data-suppression note before answering the area question.",
    ),
    dropdown(
      "dq-residence-source",
      "What do your population numbers count?",
      "A plain \"population\" table is almost always total population; look for \"employed\" or an age breakdown for the higher rungs.",
    ...
    dropdown(
      ...
      "Answer with the smallest area in your source's own table, using the coarsest grain that adds up to official totals (data suppression can hollow out fine tables).",
      GRANULARITY_FORM_LABELS,
      false,
    ),
    dropdown(
      ...
      "Answer independently from the workplace placement — many pipelines place jobs and people differently.",
      RESIDENT_PLACEMENT_FORM_OPTIONS.map((option) => option.formLabel),
      false,
    ),
    spacer(),
    markdown(
      "## Commute flows\n\nWho commutes from where to where. Absence is normal — estimates that are forced to add up to real measured totals count for more than unchecked estimates.",
    ),
    dropdown(
      "dq-od",
      "Do you have data about where people commute from and to?",
      "Any commuting or origin-destination table counts; describe it in Methodology if none of the options fit.",
    ...
    dropdown(
      ...
      "Only answer if the previous question selected one of the first three options — the areas the commute table's origins and destinations refer to. Leave blank otherwise.",
      GRANULARITY_FORM_LABELS,
      false,
    ),
    spacer(),
    ...
    textarea(
      "methodology",
      "Methodology",
      "The questions above classify your data; use this space to describe the overall pipeline — sources, processing steps, data vintages, and anything the dropdowns couldn't capture. Reviewers read this first, and it is how ambiguous answers are resolved favorably rather than conservatively.",
      { required: true },
    ),
    textarea(
      "sources",
