...
export const DATA_QUALITY_TIER_GRADES: Record<DataQualityTier, string> = {
  "very-high": "A",
  high: "B",
  medium: "C",
  low: "D",
 ...
 */
export const DATA_QUALITY_TIER_THRESHOLDS: ReadonlyArray<{
...
export function tierForWeightedScore(weightedScore: number): ScoredDataQualityTier {
  for (const { tier, min } of DATA_QUALITY_TIER_THRESHOLDS) {
    if (weightedScore >= min) return tier;
  }
  return "absent";
}
 ...
 */
export const TIER_TO_SOURCE_QUALITY: Record<ScoredDataQualityTier, SourceQuality> = {
...
export function collapseTier(tier: DataQualityTier): SourceQuality | null {
...
export const WORKPLACE_COUNT_LADDER = [
  {
    ...
    description:
      "Physical, instrument-measured: a census place-of-work question or establishment census measures where each employee physically works",
  },
  {
    value: "physical_inferred",
    weight: 0.85,
    formLabel:
      "Government statistics reconstructed to actual work locations from linked records",
    description:
      "Physical, inferred: a register the statistics office has cross-referenced to physical work location",
  },
  {
    value: "registered_self_declared",
    weight: 0.7,
    formLabel:
      "A government business register — jobs counted at each company's registered address",
    description:
      "Registered, self-declared: counts taken at the firm's declared establishment/HQ address; multi-site firms collapse onto one node",
  },
  {
    value: "size_bands",
    weight: 0.5,
    ...
    description:
      "Institutional worker sizes: firms binned by employee-count band per area — a size distribution, not a headcount at a location",
  },
  {
    ...
    description:
      "Estimated, census-anchored proxy: magnitude bootstrapped from a real published figure then redistributed by an independent signal",
  },
  {
    value: "none",
    weight: 0,
    ...
    description:
      "No census anchor, or a residence employment count used verbatim as workplace",
  },
] as const satisfies readonly DataQualityLadderRung[];
...
export const RESIDENT_COUNT_LADDER = [
  {
    ...
    description:
      "Counts people who have a job, at their place of residence — exactly the origin set that generates commutes",
  },
  {
    value: "working_age",
    weight: 0.7,
    ...
    description:
      "Counts everyone of working age living in the area, employed or not — approximate to the commuter pool",
  },
  {
    ...
    description:
      "Counts every resident — the origin total is materially larger and more skewed than the true commuter set",
  },
  {
    value: "none",
    weight: 0,
...
export const SPATIAL_RESOLUTION_LADDER = [
  {
    ...
    formLabel: null,
    description:
      "Exact building footprints: an authoritative national cadastre / building register places mass to the individual structure",
  },
  {
    ...
    formLabel: null,
    description:
      "Uniform census mesh ≤125 m, OR an official ADM5 unit (census block / sub-里) carrying a measured per-cell count",
  },
  {
    value: "mesh_250",
    weight: 0.85,
  ...
  },
  {
    ...
    formLabel: null,
    description:
      "ML / satellite or multi-source footprints (Overture, GHS-OBAT, …) — near-complete coverage, geometry inferred, no native use-tags",
  },
  {
    ...
    formLabel: null,
    description:
      "Crowd-sourced OSM footprints alone — accurate where present but incomplete and uneven",
  },
  {
    value: "mesh_1km",
    weight: 0.5,
...
export const INTENSITY_FIDELITY_LADDER = [
  {
    ...
    formLabel: null,
    description:
      "Directly measured per cell / building: the unit already carries its own measured count, nothing is apportioned",
  },
  {
    ...
    formLabel: null,
    description:
      "Fine building-type weights (NACE-class or dwelling-level), with per-type weights tuned to the country's own published figures",
  },
  {
    ...
    formLabel: null,
    description:
      "Fine building-type weights from generic or international rules of thumb, not locally verified",
  },
  {
    value: "coarse_sector",
    weight: 0.5,
    formLabel: null,
    description:
      "Coarse sector split (primary/secondary/tertiary or office/industrial/retail); within a sector, mass spread by size",
  },
  {
    value: "binary_split",
    weight: 0.4,
    formLabel: null,
    description:
      "Binary worker/resident split: mass routed onto the correct side and spread by size, no productive types distinguished",
  },
  {
    value: "size_only",
    weight: 0.25,
    formLabel: null,
    description:
      "No building-type distinction at all — demand spread by raw building size across every building",
  },
  {
    value: "uniform",
    weight: 0.1,
...
export const OD_METRIC_LADDER = [
  ...
  },
  {
    ...
    description:
      "Per-area in/out totals plus distance/containment shares or destination pins",
  },
  {
    value: "marginal_od",
    weight: 0.5,
  ...
  },
  {
    ...
    description:
      "Synthetic flows doubly-constrained (IPF) to real measured origin and destination totals",
  },
  {
    ...
    formLabel: null,
    description:
      "Synthetic marginals with flows IPF-bounded under an informed prior calibrated from analogue countries; reviewer distinguishes from 'none'",
  },
  {
    value: "none",
    weight: 0,
...
export function ladderWeight<V extends string>(
...
export const WORKPLACE_PLACEMENT_FORM_OPTIONS: readonly PlacementFormOption[] = [
  {
  ...
  {
    formLabel:
      "Official building footprints, split by workplace type with densities tuned to match real statistics",
...
export const RESIDENT_PLACEMENT_FORM_OPTIONS: readonly PlacementFormOption[] = [
  {
    formLabel: "Nothing to place — the data is already per building or block",
    resolution: "mesh_125_or_adm5",
    intensity: "measured_per_unit",
  ...
  {
    formLabel:
      "Official building footprints with per-building dwelling counts or residential floor area",
    resolution: "exact_footprints",
    intensity: "fine_types_calibrated",
  },
  {
