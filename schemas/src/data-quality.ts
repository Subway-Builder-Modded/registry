...
function ladderEnum<V extends string>(
  ladder: readonly DataQualityLadderRung<V>[],
): z.ZodEnum<[V, ...V[]]> {
  return z.enum(ladder.map((rung) => rung.value) as [V, ...V[]]);
}

export const DataQualityTierSchema = z.enum(DATA_QUALITY_TIERS);

const SCORED_TIERS = DATA_QUALITY_TIERS.filter(
  (tier): tier is ScoredDataQualityTier => tier !== "unknown",
) as [ScoredDataQualityTier, ...ScoredDataQualityTier[]];
...
export type DataQualityProvenanceMethod =
  (typeof DATA_QUALITY_PROVENANCE_METHODS)[number];
