...
| HU            | Yukina-  | physical (KSH census place-of-work)         | Overture / GULU building use + NACE fit                          | ADM3 (település)                          | employed_resident (KSH + cadastre)         | census blocks (1.08M measured) + cadastre parcels  | ADM3 (település)                           | Structured marginals (county OD + containment bins) | ADM3 (település)    |
...
| HU            | Yukina-  | 0.42      | 0.63     | 0.53 | 0.51          | **0.59**           | Medium    |
...
- **Medium — HU.** KSH census place-of-work on Overture/GULU footprints + NACE fit (ADM3) · employed residents on the census enumeration-block layer (1.08M measured blocks) + cadastre parcels (ADM3 anchor) · structured marginals (county O/D + containment bins, ADM3).
  - _Raw_ — workplace `1.0 × (0.7×0.85) × 0.70 = 0.417`, resident `1.0 × (0.9×1.0) × 0.70 = 0.63`, od `0.75 × 0.70 = 0.525` → **0.51**. _(Resident R 0.7 → 0.9 at first release, 2026-09-22: residents ship on the measured census enumeration-block layer + per-parcel counts — the same measured-sub-municipal-layer rung as CZ/SK/LV; the ADM3 anchor grain is unchanged per the grain-of-the-count convention.)_
  - _Weighted_ — workplace `0.798 × 0.70 = 0.558`, resident `0.95 × 0.70 = 0.665`, od `0.525` → **0.59**.
