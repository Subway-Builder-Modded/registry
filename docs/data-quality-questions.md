...
| Building footprints, split by workplace type using standard assumptions                             | Same idea, but the densities come from rules of thumb rather than the country's own data                                                          |
...
| Building footprints, with homes identified and weighted using standard assumptions       | Buildings are classified as residential by type or shape, and people are spread among them using generic occupancy assumptions (e.g. an assumed number of residents per dwelling or per m²) rather than the country's own housing statistics |
