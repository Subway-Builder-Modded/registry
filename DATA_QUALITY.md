...
3. **When it remains ambiguous, take the middle and record the bounds** so the call is auditable. _Example:_ Mexico's workplace magnitude is measured as an ~ADM2 (municipal) industry total, but the establishment directory carries real employee-size bands at address level (ADM5). Within the rubric, it is thus scored at **~ADM4**, between the coarser ADM2 (total-only) and the finer ADM5 (treat the establishments as measured units).
...
| Census mesh ≤ 250 m                                     | 0.85   | A fine, complete published grid cell (building-footprint refinement within the cell lifts toward 1.0)                                                                                                                              | JP 250m / 500m→100m, LT 250m                                               |
